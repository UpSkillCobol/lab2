      *    The FileStat-Msgs.cpy copybook contains an EVALUATE
      *    statement to translate the two-digit file status codes that
      *    may be generated by file I/O statements.

      *    The copybook assumes that the file status data item name is
      *    "STATUS" and the error message data item is named "MSG".
      *    By using the COPY statement’s REPLACING clause, however, you
      *    may use the data names you wish, as follows:

      *    COPY FileStat-Msgs
      *    REPLACING
      *        STATUS BY file-status-data-item-name
